  ...
  switch (action.type) {
    ...
    case "IS_USER_REGISTERED_TO_COURSE":
      ...
      const found =
        ...
        courseFoundClone.user.registered_course &&
    ...
    case "GET_USER_COURSE_lIST":
      console.log(state);
      return state;
    case "GET_USER_COURSE_lIST_SUCCESS":
      const courseList = { ...state };
      courseList.user.courseLists = payload.payload;
      courseList.user.students = [];
      payload.payload.forEach((course) => {
        course.students.forEach((student) => {
          courseList.user.students.push(student);
        });
      });

      return courseList;

    case "GET_USER_COURSE_lIST_ERROR":
      console.log(state);
      return state;
