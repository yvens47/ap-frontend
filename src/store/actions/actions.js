...
export function resetFilteredCourse() {
  return {
    type: "RESET_FILTER_COURSE"
  };
}
...
export function getUserCourseList(userid) {
  return (dispatch) => {
    dispatch({ type: "GET_USER_COURSE_lIST" });
    axios
      .get(
      ...
      .then((response) => {
        if (response.data.error) {
          dispatch({
            type: "GET_USER_COURSE_lIST_ERROR",
            payload: response.data.error
          });
        } else {
          dispatch({
            type: "GET_USER_COURSE_lIST_SUCCESS",
            payload: response.data
          });
        }
      })
